...
class VideoMMEAdapter(DatasetAdapter):
    ...
    default_hf_repo = "lmms-lab/Video-MME"
    default_hf_config = "videomme"
    ...
    def format_sample(self, raw_item, split, args):
        ...
        media = ensure_local_media_path(get_first(raw_item, ["video", "video_path", "video_id", "youtube_id"]), args.video_root or args.media_root)
        return {
            "id": str(get_first(raw_item, ["question_id", "id", "sample_id"], default="")),
            "task_name": normalize_text(get_first(raw_item, ["domain", "duration", "task_name"], default=self.name)),
            "media_type": "video",
            "media": media,
